...
func TestTable_filterNode(t *testing.T) {
	...
	enrFilter, _ := ParseEthFilter("oiz")
	...
	var r2 enr.Record
	r2.Set(enr.WithEntry("eth", eth{ForkID: forkid.NewID(params.OIZChainConfig, core.DefaultChapelGenesisBlock().ToBlock(), uint64(0), uint64(0))}))
	if enrFilter(&r2) {
	...
	var r3 enr.Record
	r3.Set(enr.WithEntry("eth", eth{ForkID: forkid.NewID(params.OIZChainConfig, core.DefaultOIZGenesisBlock().ToBlock(), uint64(0), uint64(0))}))
	if !enrFilter(&r3) {
